...
namespace Spring.Net.Example.Dao.Common
{
    ...
    public static class DataReaderExtensions
    {
        ...
        public static T SelectOne<T>(this IDataReader reader)
            where T : class, new()
        {
            if (reader == null)
                throw new ArgumentNullException("reader");
            return EntityConverter<T>.SelectOne(reader);
        }
        public static T ConvertToEntity<T>(this IDataReader reader)
            where T : class, new()
        {
            T result = new T();
            foreach (var p in typeof(T).GetProperties())
            {
                for (int i = 0; i < reader.FieldCount; i++)
                {
                    ...
                    if (columName.Equals(p.Name, StringComparison.CurrentCultureIgnoreCase))
                    {
                        if (val != DBNull.Value && val != null)
                        {
                            if (p.PropertyType == typeof(int))
                            {
                                p.SetValue(result, int.Parse(val.ToString()));
                            }
                            else if (p.PropertyType == typeof(double))
                            {
                                p.SetValue(result, double.Parse(val.ToString()));
                            }
                            else if (p.PropertyType == typeof(decimal))
                            {
                                p.SetValue(result, decimal.Parse(val.ToString()));
                            }
                            else if (p.PropertyType == typeof(DateTime))
                            {
                                p.SetValue(result, DateTime.Parse(val.ToString()));
                            }
                            else if (p.PropertyType == typeof(string))
                            {
                                p.SetValue(result, val.ToString());
                            }
                            else
                            {
            ...
            where T : class, new()
        {
            ...
            private struct DbColumnInfo
            {
                ...
                public DbColumnInfo(PropertyInfo prop, DbColumnAttribute attr = null)
                {
            ...
            #endregion Struct: DbColumnInfo

            #region Fields

            private static Converter<IDataReader, List<T>> batchDataLoader;
            private static Converter<IDataReader, T> singleDataLoader;
            ...
            private static Converter<IDataReader, T> SingleDataLoader
            {
                get
                {
                    singleDataLoader = CreateSingleDataLoader(new List<DbColumnInfo>(GetProperties()));
            ...
            private static Converter<IDataReader, T> CreateSingleDataLoader(List<DbColumnInfo> columnInfoes)
            {
            ...
            internal static T SelectOne(IDataReader reader)
            {
                return SingleDataLoader(reader);
            }
        ...
        #endregion Class: EntityConverter<T>
    }
}